...
public class GuessSystem {
	...
	public GuessSystem() throws Exception {
		...
		system.setupSynthesizer(new WindowsSynthesizer(), Gender.MALE);
		
		// Add the flow
		system.addModule(new FlowModule(new GuessFlow()));
		
		// Load a grammar in the recognizer
		system.loadContext("default", new SpeechGrammarContext(new SRGSGrammar(getClass().getResource("GuessGrammar.xml").toURI())));
